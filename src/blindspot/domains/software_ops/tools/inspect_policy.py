"""Software-ops inspect-policy tool."""
...
class SOInspectPolicyArgs(BaseModel):
    model_config = ConfigDict(extra="forbid")
    policy_id: str | None = None


class SoftwareOpsInspectPolicyTool(PythonTool[SOInspectPolicyArgs, BaseModel]):
    specification = ToolSpecification(
        tool_id=ComponentID(namespace="software-ops", name="inspect-policy", version="1.0.0"),
        display_name="Inspect Policy",
        description="Return human-readable software-ops policy information.",
        read_scopes=["policies"],
        write_scopes=[],
        side_effect_level="none",
        idempotent=True,
    )
    args_model = SOInspectPolicyArgs
