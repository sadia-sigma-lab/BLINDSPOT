"""E-commerce inspect-policy tool."""
...
class ECInspectPolicyArgs(BaseModel):
    model_config = ConfigDict(extra="forbid")
    policy_id: str | None = None


class ECInspectPolicyTool(PythonTool[ECInspectPolicyArgs, BaseModel]):
    specification = ToolSpecification(
        tool_id=ComponentID(namespace="core", name="ec-inspect-policy", version="1.0.0"),
        display_name="Inspect E-commerce Policy",
        description="Return human-readable e-commerce policy information.",
        read_scopes=["policies"],
        write_scopes=[],
        side_effect_level="none",
        idempotent=True,
    )
    args_model = ECInspectPolicyArgs
    output_model = BaseModel

    def read(self, view: ReadOnlyStateView, args: ECInspectPolicyArgs, context: ToolExecutionContext) -> Any:
    ...
    def plan_mutations(self, view: ReadOnlyStateView, args: ECInspectPolicyArgs, context: ToolExecutionContext) -> MutationPlan:
        return MutationPlan()
